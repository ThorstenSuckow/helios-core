...
public:
    ...
    TypeErasedInlineValueWrapper(TypeErasedInlineValueWrapper&& other) noexcept : typeId_(other.typeId_) {
        moveFrom(other);
    };
    ...
    template <typename TConcrete>
        requires(!std::same_as<std::remove_cvref_t<TConcrete>, TypeErasedInlineValueWrapper>) &&
                std::is_rvalue_reference_v<TConcrete&&> && (!std::is_const_v<std::remove_reference_t<TConcrete>>)
    explicit TypeErasedInlineValueWrapper(TConcrete&& concrete) noexcept
        ...
        destroy_ = [](Concept* model) noexcept { std::destroy_at(static_cast<ModelType*>(model)); };

        move_ = [](Concept* source, void* destination) noexcept -> Concept* {
            auto* sourceModel = static_cast<ModelType*>(source);
            auto* targetModel =
                std::construct_at(static_cast<ModelType*>(destination), std::move(sourceModel->concreteData_));
