...
export module helios.core.log.LogManager;
...
public:
    ...
    [[nodiscard]] Logger& registerLogger(const std::string& scope) noexcept {
        ...
        {
            std::scoped_lock sinkLock(sinkMutex_);
            for (const auto& sink : registeredSinks_) {
                if (sink && enabledSinks_.contains(sink->typeId())) {
     ...
     */
    void registerSink(const std::shared_ptr<LogSink>& sink) {
        registerSink(sink, true);
    }
     ...
     */
    void registerSink(const std::shared_ptr<LogSink>& sink, bool enabled) {
        if (!sink) {
            return;
        }

        std::scoped_lock lock(sinkMutex_);
    ...
    void enableSink(Args&&... args)
        requires std::derived_from<TSink, LogSink>
    {
     ...
     */
    void enableSink(const std::shared_ptr<LogSink>& sink) {
        if (!sink) {
            return;
        }
     ...
     */
    void disableSink(const std::shared_ptr<LogSink>& sink) {
        if (!sink) {
            return;
        }
